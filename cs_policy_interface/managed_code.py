...
class ManagedCode(object):
            ...
            for region in regions:
                if check_type == 'Snapshot_encrypted':
                    try:
                        ...
                        if 'invalid user id' in str(e).lower():
                            continue
                        raise Exception(
                            'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                                str(e)))
    ...
    def aws_elb_health_check_enabled(self, **kwargs):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
            for region in [region.get('id') for region in self.execution_args['regions']]:
                try:
                    elb_instances = run_aws_operation(credentials, 'elb', 'describe_load_balancers',
                                                      region_name=region,
                                                      response_key='LoadBalancerDescriptions')
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
                evaluated_resources += 1
                for elb_instance in elb_instances:
                    if not elb_instance.get('HealthCheck'):
                        output.append(OrderedDict(
                            ResourceId=elb_instance.get('LoadBalancerName'),
                            ResourceName=elb_instance.get('LoadBalancerName'),
                            ResourceType='Load_Balancers'))
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))

    def aws_ec2_security_group_naming_convention(self, **kwargs):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
            ...
            for region in [region.get('id') for region in self.execution_args['regions']]:
                try:
                    evaluated_resources += 1
                    security_groups = run_aws_operation(credentials, 'ec2', 'describe_security_groups',
                                                        region_name=region,
                                                        response_key='SecurityGroups')
                except Exception as e:
                    raise Exception(
    ...
    def aws_config_region_violation(self):
        output = list()
        evaluated_resources = 0
        aws_resource_types = [
            ...
            'AWS::EC2::FlowLog',
            'AWS::EC2::VPCPeeringConnection', 'AWS::Elasticsearch::Domain', 'AWS::IAM::Group', 'AWS::IAM::Policy',
            'AWS::IAM::Role', 'AWS::IAM::User', 'AWS::ElasticLoadBalancingV2::LoadBalancer', 'AWS::ACM::Certificate',
            'AWS::RDS::DBInstance', 'AWS::RDS::DBSubnetGroup', 'AWS::RDS::DBSecurityGroup', 'AWS::RDS::DBSnapshot',
            'AWS::RDS::DBCluster', 'AWS::RDS::DBClusterSnapshot', 'AWS::RDS::EventSubscription', 'AWS::S3::Bucket',
            'AWS::S3::AccountPublicAccessBlock', 'AWS::Redshift::Cluster', 'AWS::Redshift::ClusterSnapshot',
            ...
            'AWS::Lambda::Function', 'AWS::NetworkFirewall::Firewall', 'AWS::NetworkFirewall::FirewallPolicy',
            ...
            'AWS::WAFv2::WebACL', 'AWS::WAFv2::RuleGroup', 'AWS::WAFv2::IPSet', 'AWS::WAFv2::RegexPatternSet',
            ...
            'AWS::ServiceCatalog::CloudFormationProduct', 'AWS::ServiceCatalog::Portfolio', 'AWS::SQS::Queue',
            'AWS::KMS::Key', 'AWS::QLDB::Ledger', 'AWS::SecretsManager::Secret', 'AWS::SNS::Topic',
            'AWS::SSM::FileData', 'AWS::Backup::BackupPlan', 'AWS::Backup::BackupSelection', 'AWS::Backup::BackupVault',
            'AWS::Backup::RecoveryPoint', 'AWS::ECR::Repository', 'AWS::ECS::Cluster', 'AWS::ECS::Service',
            'AWS::ECS::TaskDefinition', 'AWS::EFS::AccessPoint', 'AWS::EFS::FileSystem', 'AWS::EKS::Cluster',
            'AWS::OpenSearch::Domain', 'AWS::EC2::TransitGateway', 'AWS::Kinesis::Stream',
            'AWS::Kinesis::StreamConsumer', 'AWS::CodeDeploy::Application', 'AWS::CodeDeploy::DeploymentConfig',
            ...
            'AWS::ElasticLoadBalancingV2::Listener', 'AWS::StepFunctions::StateMachine', 'AWS::Batch::JobQueue',
            'AWS::Batch::ComputeEnvironment', 'AWS::AccessAnalyzer::Analyzer', 'AWS::Athena::WorkGroup',
            'AWS::Athena::DataCatalog', 'AWS::Detective::Graph', 'AWS::GlobalAccelerator::Accelerator',
            'AWS::GlobalAccelerator::EndpointGroup', 'AWS::GlobalAccelerator::Listener',
            'AWS::EC2::TransitGatewayAttachment', 'AWS::EC2::TransitGatewayRouteTable', 'AWS::DMS::Certificate',
            'AWS::AppConfig::Application', 'AWS::AppSync::GraphQLApi', 'AWS::DataSync::LocationSMB',
            'AWS::DataSync::LocationFSxLustre', 'AWS::DataSync::LocationS3', 'AWS::DataSync::LocationEFS',
            'AWS::DataSync::Task', 'AWS::DataSync::LocationNFS', 'AWS::EC2::NetworkInsightsAccessScopeAnalysis',
            'AWS::EKS::FargateProfile', 'AWS::Glue::Job', 'AWS::GuardDuty::ThreatIntelSet', 'AWS::GuardDuty::IPSet',
            'AWS::SageMaker::Workteam', 'AWS::SageMaker::NotebookInstanceLifecycleConfig',
            'AWS::ServiceDiscovery::Service', 'AWS::ServiceDiscovery::PublicDnsNamespace', 'AWS::SES::ContactList',
            'AWS::SES::ConfigurationSet', 'AWS::Route53::HostedZone '
        ]
        try:
            credentials = self.execution_args['auth_values']
            usr_input_aws_regions = self.execution_args['args'].get('aws_regions')
            for region in [region.get('id') for region in self.execution_args['regions']]:
                for resource_type in aws_resource_types:
                    discovered_resources = run_aws_operation(credentials, 'config', 'list_discovered_resources',
                                                             region_name=region,
                                                             operation_args={'resourceType': resource_type},
                                                             response_key='resourceIdentifiers')
                    evaluated_resources += 1
                    for discovered_resource in discovered_resources:
                        config_items = run_aws_operation(credentials, 'config', 'get_resource_config_history',
                                                         region_name=region,
                                                         operation_args={
                                                             'resourceType': discovered_resource.get('resourceType'),
                                                             'resourceId': discovered_resource.get('resourceId'),
                                                         },
                                                         response_key='configurationItems')
                        evaluated_resources += 1
                        if not config_items:
                            break
                        for config_item in config_items:
                            if config_item.get('awsRegion') not in usr_input_aws_regions:
                                output.append(OrderedDict(
                                    ResourceId=discovered_resource.get('resourceType'),
                                    ResourceName=discovered_resource.get('resourceId'),
    ...
    def aws_access_analyzer_findings_enabled(self):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
            for region in [region.get('id') for region in self.execution_args['regions']]:
                try:
                    evaluated_resources += 1
                    analyzers = run_aws_operation(credentials, 'accessanalyzer', 'list_analyzers',
                                                  region_name=region,
                                                  response_key='analyzers')
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
                for analyzer in analyzers:
                    if analyzer.get('status') != 'ACTIVE':
                        output.append(OrderedDict(
                            ResourceId=analyzer.get('arn'),
                            ResourceName=analyzer.get('name'),
                            ResourceType='IAM'
                        ))
            return output, evaluated_resources
        except Exception as e:
    ...
    def aws_ec2_unrestricted_smtp_access(self, **kwargs):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
            for region in [region.get('id') for region in self.execution_args['regions']]:
                try:
                    security_groups_descriptions = run_aws_operation(credentials, 'ec2', 'describe_security_groups',
                                                                     region_name=region,
                                                                     response_key='SecurityGroups')
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
                evaluated_resources += 1
                for security_groups_description in security_groups_descriptions:
                    for ip_permission in security_groups_description.get('IpPermissions', []):
                        if ip_permission.get('FromPort') == 25 and ip_permission.get('ToPort') == 25:
                            for ip_ranges in ip_permission.get('IpRanges'):
                                if ip_ranges.get('CidrIp') == '0.0.0.0/0':
                                    output.append(OrderedDict(
                                        ResourceId=security_groups_description.get('GroupId'),
                                        ResourceName=security_groups_description.get('GroupName'),
                                        ResourceType='Load_Balancers'))
                            for ip_ranges in ip_permission.get('Ipv6Ranges'):
                                if ip_ranges.get('CidrIpv6') == '::/0':
                                    output.append(OrderedDict(
                                        ResourceId=security_groups_description.get('GroupId'),
                                        ResourceName=security_groups_description.get('GroupName'),
                                        ResourceType='Load_Balancers'))
        ...
        try:
            ...
            for region in regions:
                ...
                for images in ec2_response.get('Images', []):
                    ...
                    # print(images)
                    # break
                    for tag in images.get('Tags', []):
                        if tag.get('Key') == 'Name':
                            if not re.search(pattern, tag.get('Value', '')):
                                output.append(
                                    OrderedDict(
                                        ResourceId=images['ImageId'],
                                        ResourceName=images['ImageId'],
            ...
            for region in regions:
                ...
                for alb in alb_response.get('LoadBalancers', []):
                    ...
                    listener_info = run_aws_operation(
                        ...
                        'elbv2',
                        'describe_listeners',
                        operation_args={'LoadBalancerArn': alb.get('LoadBalancerArn')},
                        region_name=region)
            ...
            for region in [region.get('id') for region in self.execution_args['regions']]:
                try:
                    ec2_response = run_aws_operation(
                        ...
                        'describe_images',
                        region_name=region,
                        operation_args=dict(Owners=['self'])
                    )
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
    ...
    def azure_elastic_pool_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            try:
                inventory_db = get_mongo_client(self.connection_args)[self.connection_args['database_name']]
            except Exception as e:
                raise Exception('Unable to connect to the db. Error {}'.format(str(e)))
            query = {"service_account_id": service_account_id,
                     "resource_type": "Servers",
                     "resource": "Elastic_Pool_Database",
                     "is_deleted": False
                     }
            elastic_pool_db_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.ElasticPoolsList", query)
            orphaned_elastic_pool_db = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Servers",
                            "resource": "Elastic_Pools",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": elastic_pool_db_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
            ...
            if "output" in orphaned_elastic_pool_db[0]:
                output = orphaned_elastic_pool_db[0]["output"]
            if "evaluated_resource" in orphaned_elastic_pool_db[0]:
                evaluated_resources = orphaned_elastic_pool_db[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
    ...
    def azure_sql_managed_instance_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            try:
                ...
                raise Exception('Unable to connect to the db. Error {}'.format(str(e)))
            orphaned_managed_instance_query = {"service_account_id": service_account_id,
                                               "resource_type": "Azure_SQL_Managed_Instance",
                                               "resource": "SQL_Managed_Instances_DB",
                                               "is_deleted": False}
            orphaned_managed_instance_list = inventory_db["service_resource_inventory"].distinct(
                "check_resource_element", orphaned_managed_instance_query)

            orphaned_managed_instance = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Azure_SQL_Managed_Instance",
                            "resource": "SQL_Managed_Instances",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"$or": [
                            {"meta_data.ListSqlManagedInstancesDatabases": {"$nin": orphaned_managed_instance_list}},
                            {"meta_data.ListSqlManagedInstancesDatabases": {"$exists": False}}]}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_managed_instance[0]:
                output = orphaned_managed_instance[0]["output"]
            if "evaluated_resource" in orphaned_managed_instance[0]:
                evaluated_resources = orphaned_managed_instance[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_app_service_plan_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            ...
            orphaned_app_service_plan = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "App_Services",
                            "resource": "Appservice_Plan",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"summary_details.properties.numberOfSites": 0}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_app_service_plan[0]:
                output = orphaned_app_service_plan[0]["output"]
            if "evaluated_resource" in orphaned_app_service_plan[0]:
                evaluated_resources = orphaned_app_service_plan[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_cosmos_mongo_db_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            try:
                ...
                raise Exception('Unable to connect to the db. Error {}'.format(str(e)))
            collections_query = {"service_account_id": service_account_id,
                                 "resource_type": "Cosmos_DB",
                                 "resource": "MongoDB_Collections",
                                 "is_deleted": False
                                 }
            collection_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.MongoDatabasesList", collections_query)
            orphaned_mongo_servers = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Cosmos_DB",
                            "resource": "Mongo_Databases",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": collection_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                        ...
                        "ResourceName": "$summary_details.name",
                        "ResourceCategory": "$category",
                        "MinimumThroughput": "$extended_summary_details.properties.minimumThroughput",
                        "Throughput": "$extended_summary_details.properties.throughput"
                    }}],
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_mongo_servers[0]:
                output_detail = orphaned_mongo_servers[0]["output"]
                for detail in output_detail:
                    if "MinimumThroughput" in detail.keys():
                        if int(detail.get('MinimumThroughput', 0)) <= int(detail.get('Throughput', 0)):
                            detail.pop("MinimumThroughput")
                            detail.pop("Throughput")
                            output.append(detail)
            if "evaluated_resource" in orphaned_mongo_servers[0]:
                evaluated_resources = orphaned_mongo_servers[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_cosmos_cassandra_keyspace_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            ...
            query = {"service_account_id": service_account_id,
                     "resource_type": "Cosmos_DB",
                     "resource": "Cassandra_Keyspace_Table",
                     "is_deleted": False
                     }
            cassandra_keyspace_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.CassandraKeySpaceList", query)

            orphaned_cassandra_keyspace = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Cosmos_DB",
                            "resource": "Cassandra_Keyspace",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": cassandra_keyspace_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_cassandra_keyspace[0]:
                output_detail = orphaned_cassandra_keyspace[0]["output"]
                for detail in output_detail:
                    if "MinimumThroughput" in detail.keys():
                        if int(detail.get('MinimumThroughput', 0)) <= int(detail.get('Throughput', 0)):
                            detail.pop("MinimumThroughput")
                            detail.pop("Throughput")
                            output.append(detail)
            if "evaluated_resource" in orphaned_cassandra_keyspace[0]:
                evaluated_resources = orphaned_cassandra_keyspace[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_cosmos_sql_db_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            ...
            query = {"service_account_id": service_account_id,
                     "resource_type": "Cosmos_DB",
                     "resource": "Cosmos_SQL_Containers",
                     "is_deleted": False
                     }
            sql_table_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.CosmosSQLDatabasesList", query)

            orphaned_sql_db = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Cosmos_DB",
                            "resource": "Cosmos_SQL_Databases",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": sql_table_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_sql_db[0]:
                output_detail = orphaned_sql_db[0]["output"]
                for detail in output_detail:
                    if "MinimumThroughput" in detail.keys():
                        if int(detail.get('MinimumThroughput', 0)) <= int(detail.get('Throughput', 0)):
                            detail.pop("MinimumThroughput")
                            detail.pop("Throughput")
                            output.append(detail)
            if "evaluated_resource" in orphaned_sql_db[0]:
                evaluated_resources = orphaned_sql_db[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_cosmos_gremlin_db_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        try:
            service_account_id = self.execution_args["service_account_id"]
            ...
            query = {"service_account_id": service_account_id,
                     "resource_type": "Cosmos_DB",
                     "resource": "Gremlin_Graph",
                     "is_deleted": False
                     }
            gremlin_graph_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.GremlinDatabaseList", query)
            orphaned_gremlin_db = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Cosmos_DB",
                            "resource": "Gremlin_Databases",
                            "is_deleted": False
                            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": gremlin_graph_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_gremlin_db[0]:
                output_detail = orphaned_gremlin_db[0]["output"]
                for detail in output_detail:
                    if "MinimumThroughput" in detail.keys():
                        if int(detail.get('MinimumThroughput', 0)) <= int(detail.get('Throughput', 0)):
                            detail.pop("MinimumThroughput")
                            detail.pop("Throughput")
                            output.append(detail)
            if "evaluated_resource" in orphaned_gremlin_db[0]:
                evaluated_resources = orphaned_gremlin_db[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def aws_target_mirror_sessions_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        orphaned_target_mirror = list()
        try:
            service_account_id = self.execution_args["service_account_id"]
            try:
                inventory_db = get_mongo_client(self.connection_args)[self.connection_args['database_name']]
                ...
                raise Exception('Unable to connect to the db. Error {}'.format(str(e)))
            nic_query = {"service_account_id": service_account_id,
                         "resource_type": "VPC",
                         "resource": "Network_Interfaces",
                         "is_deleted": False,
                         "summary_details.attachment.instanceId": {"$exists": False}}
            nic_list = inventory_db["service_resource_inventory"].distinct(
                "check_resource_element", nic_query)
            if nic_list:
                target_list = inventory_db["service_resource_inventory"].distinct("check_resource_element", {
                    "service_account_id": service_account_id,
                    "resource_type": "Traffic_Mirroring",
                    "resource": "Mirror_Target",
                    "is_deleted": False,
                    "summary_details.networkInterfaceId": {"$in": nic_list}
                })
                if target_list:
                    orphaned_target_mirror = list(inventory_db["service_resource_inventory"].aggregate([
                        {"$match": {"service_account_id": service_account_id,
                                    "resource_type": "Traffic_Mirroring",
                                    "resource": "Mirror_Sessions",
                                    "is_deleted": False
                                    }},
                        {"$facet": {"output": [{
                            "$match":
                                {"summary_details.trafficMirrorTargetId": {"$in": target_list}}},
                            {"$project": {
                                "_id": 0,
                                "Resource": "$resource",
                                "ResourceType": "$resource_type",
                            ...
                            "evaluated_resource": [{"$count": "totalRecords"}]}}]))
                    if "output" in orphaned_target_mirror[0]:
                        output = orphaned_target_mirror[0]["output"]
                    if "evaluated_resource" in orphaned_target_mirror[0]:
                        evaluated_resources = orphaned_target_mirror[0]["evaluated_resource"]
                        if len(evaluated_resources) > 0:
                            evaluated_resources = evaluated_resources[0]["totalRecords"]
                        else:
                            evaluated_resources = 0
    ...
    def aws_source_mirror_sessions_orphaned(self, *kwargs):
        output = list()
        evaluated_resources = 0
        orphaned_source_mirror = list()
        try:
            service_account_id = self.execution_args["service_account_id"]
            try:
                inventory_db = get_mongo_client(self.connection_args)[self.connection_args['database_name']]
            ...
            nic_list = inventory_db["service_resource_inventory"].distinct(
                "check_resource_element", nic_query)
            orphaned_source_mirror = list(inventory_db["service_resource_inventory"].aggregate([
                {"$match": {"service_account_id": service_account_id,
                            "resource_type": "Traffic_Mirroring",
                            "resource": "Mirror_Sessions",
                            "is_deleted": False
                ...
                {"$facet": {"output": [{
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_source_mirror[0]:
                output = orphaned_source_mirror[0]["output"]
            if "evaluated_resource" in orphaned_source_mirror[0]:
                evaluated_resources = orphaned_source_mirror[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_dns_forwarding_rulesets_orphaned(self, *kwargs):
        evaluated_resources = 0
        output = list()
        try:
            service_account_id = self.execution_args["service_account_id"]
            ...
            database_query = {
                "service_account_id": service_account_id,
                "resource_type": "DNS_Forwarding_Rulesets",
                "resource": "DNS_Forwarding_Rules",
                "is_deleted": False}
            parent_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.ListForwardingDNSZones", database_query)
            orphaned_rulesets = list(inventory_db["service_resource_inventory"].aggregate([{"$match": {
                "service_account_id": service_account_id,
                "resource_type": "DNS_Forwarding_Rulesets",
                "resource": "DNS_Forwarding_Rulesets",
                "is_deleted": False
            }},
                {"$facet": {"output": [{
                    "$match":
                        {"check_resource_element": {"$nin": parent_list}}},
                    {"$project": {
                        "_id": 0,
                        "Resource": "$resource",
                        "ResourceType": "$resource_type",
                    ...
                    "evaluated_resource": [{"$count": "totalRecords"}]}}]))
            if "output" in orphaned_rulesets[0]:
                output = orphaned_rulesets[0]["output"]
            if "evaluated_resource" in orphaned_rulesets[0]:
                evaluated_resources = orphaned_rulesets[0]["evaluated_resource"]
                if len(evaluated_resources) > 0:
                    evaluated_resources = evaluated_resources[0]["totalRecords"]
                else:
                    evaluated_resources = 0
    ...
    def azure_dns_resolver_endpoints_orphaned(self, *kwargs):
        ...
        try:
            ...
            database_query = {
                ...
                "is_deleted": False}
            parent_list = inventory_db["service_resource_inventory"].distinct(
                "meta_data.ListDNSResolverOutbound", database_query)
            orphaned_rulesets = list(inventory_db["service_resource_inventory"].aggregate([{"$match": {
                "service_account_id": service_account_id,
                "resource_type": "DNS_Resolvers",
                "resource": "Resolver_Outbound_Endpoints",
                "is_deleted": False
            }},
                {"$facet": {"output": [{
                    "$match":

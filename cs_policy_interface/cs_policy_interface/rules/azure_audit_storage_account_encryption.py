...
class RuleExecutor(object):
    ...
    def execute(self, **kwargs):
        ...
        try:
            ...
            bearer_token = get_azure_auth_token(credentials)
            resource_url = AzureRestApiEndpoint.list_storage_accounts.format(credentials['subscription_id'])
            ...
            for each_resource in get_response.json()['value']:
                ...
                if not each_resource.get('properties', {}).get('encryption', {}).get('services', {}).get('blob',
                                                                                                         {}).get(
                    'enabled'):

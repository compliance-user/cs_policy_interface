"""
This policy audits whether Deprecated accounts with owner permissions are in your subscription.
Deprecated accounts are accounts that have been blocked from signing in.
"""
        ...
        try:
            ...
            for each_resource in get_response.json()['value']:
                if each_resource.get('properties', {}).get('principalType', {}) == 'User' and \
                    ...
                    if not get_graph_response.json().get('accountEnabled', {}):

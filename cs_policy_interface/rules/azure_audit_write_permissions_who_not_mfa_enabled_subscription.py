"""
This policy audits whether Accounts with write permissions that are MFA disabled in subscription.
It should be done for all subscription accounts with write privileges to prevent a breach of accounts or
resources.
"""
import requests
        ...
        try:
            ...
            for each_resource in get_response.json()['value']:
                if each_resource.get('properties', {}).get('principalType', {}) == 'User' and \
                        RoleDefinitionID.CONTRIBUTOR in \
